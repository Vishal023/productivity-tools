...
import type { SprintPlannerState, Sprint, SprintMember, JiraTicket, Release } from '@/types'
...
type SprintPlannerStore = SprintPlannerState & SprintPlannerActions
  ...
  persist(
    (set, get) => ({
      ...
      addTeamMember: (name, defaultCapacity) => {
        const id = generateId()
        set(state => ({
          team: [...state.team, { id, name, defaultCapacity }]
        }))
      },
